...
#endif
static WORD PortType, BridgeRev, PortDly;
...
/* I/O control delay */
#ifdef POSIX_TTY
static struct timespec iodeley_time = {(time_t)1,(long)0};//1sec
#endif
...
#ifdef POSIX_TTY
    int err_no;
	if((err_no = nanosleep(&iodeley_time,NULL))!=0){
#ifdef DEBUG
        fprintf(stderr,"Zzzz waht? %d",err_no);
#endif //DEBUG
    }
...
static
bool read_bridge (BYTE *buffer, size_t count)
{
	size_t cnt = 0;
...
#endif //WIN32
#ifdef POSIX_TTY
    if(pc->PortClass == TY_LPT||pc->PortClass == TY_COMM||pc->PortClass == TY_RSCR){
		pc->Info1 = "Not support Pararel Port and Direct Serial programer now.\n";
		return 1;        
    }    
#endif //POSIX_TTY
...
#endif //WIN32
#ifdef POSIX_TTY
    //tty style POSIX serial port
    if(pc->PortClass == TY_BRIDGE || pc->PortClass == TY_VCOM){
        
        //open tty
        //tty = 
        if(pc->DeviceName == NULL){
            //set AT Compartible i386/x64 16450/16550 style tty name
            switch(pc->PortNum){
                case 1:
...
void close_ifport ()
{
	...
	if(tty > 0){
        ...
        if(tcsetattr(tty,TCSANOW, &old_ttyoptions) == -1){
            fprintf(stderr,"Fail to resume old tty config\n");
        }
		close(tty);
        tty = 0;
    }
