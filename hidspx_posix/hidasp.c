 ...
 */
	rc = usb_control_msg(hHID, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_ENDPOINT_IN, 
...
static int hidWrite(usb_dev_handle *h, char *buf, int Length, int id)
{
	...
	rc = usb_control_msg(hHID, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_ENDPOINT_OUT,
...
void hidFree(){
	if (hHID_DLL) FreeLibrary(hHID_DLL);
	hHID_DLL = NULL;
}
